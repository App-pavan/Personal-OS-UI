...
export function TaskThemeProvider({ children }: { children: ReactNode }) {
  const [themeId, setThemeIdState] = useState<TaskThemeId>("personal-os");

  useEffect(() => {
    const stored = window.localStorage.getItem(TASK_THEME_STORAGE_KEY) as TaskThemeId | null;
    if (stored && TASK_THEMES[stored]) setThemeIdState(stored);
  }, []);
