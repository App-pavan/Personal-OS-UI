...
export function useTaskMutations() {
  ...
  const useWrapMutation = <TArgs extends unknown[]>(
  ...
  return {
    ...
    remove: useWrapMutation((id: string) => taskService.remove(id), "Task could not be deleted."),
    bulk: useWrapMutation(
      (op: BulkTaskOperation) => taskService.bulk(op),
      "Bulk action could not be applied.",
    ),
    complete: useWrapMutation(
      (id: string) => taskService.complete(id),
      "Task could not be completed.",
    ),
    reopen: useWrapMutation((id: string) => taskService.reopen(id), "Task could not be reopened."),
    archive: useWrapMutation(
      (id: string) => taskService.archive(id),
      "Task could not be archived.",
    ),
    restore: useWrapMutation(
      (id: string) => taskService.restore(id),
      "Task could not be restored.",
    ),
    duplicate: useWrapMutation(
      (id: string) => taskService.duplicate(id),
      "Task could not be duplicated.",
    ),
    pin: useWrapMutation(
      (id: string, pinned: boolean) => taskService.setPinned(id, pinned),
      "Pin failed.",
    ),
    favorite: useWrapMutation(
      (id: string, favorite: boolean) => taskService.setFavorite(id, favorite),
      "Favorite failed.",
    ),
    progress: useWrapMutation(
      (id: string, progress: number) => taskService.setProgress(id, progress),
      "Progress could not be saved.",
    ),
    assign: useWrapMutation(
      (id: string, name: string) => taskService.assign(id, name),
      "Assignment could not be saved.",
    ),
    addSubtask: useWrapMutation(
      (id: string, title: string) => taskService.addSubtask(id, title),
      "Subtask could not be added.",
    ),
    toggleSubtask: useWrapMutation(
      (id: string, subId: string) => taskService.toggleSubtask(id, subId),
      "Subtask could not be updated.",
    ),
    toggleChecklistItem: useWrapMutation(
      (id: string, itemId: string) => taskService.toggleChecklistItem(id, itemId),
      "Item could not be updated.",
    ),
    comment: useWrapMutation(
